...
# 数据库路径
DB_PATH = os.path.join(APP_ROOT, 'database', 'tasks.db')
BACKUP_DIR = os.path.join(APP_ROOT, 'database', 'backups')
...
def check_migration_needed(cursor):
    ...
    return tasks_needs_migration, scheduled_needs_migration
...
def main():
    ...
    try:
        # 检查是否需要迁移
        print("\n步骤 3: 检查迁移需求")
        tasks_needs, scheduled_needs = check_migration_needed(cursor)
